...
impl Collect {
    ...
    pub fn __construct(name: Vec<i32> ) -> Self {
        Self { name }
    }

    #[getter]
    pub fn get_name(&self) -> Vec<i32> {
        self.name.clone()
    }

    #[setter]
    pub fn set_name(&mut self, name: Vec<i32>) {
        self.name = name;
    }
    ...
    pub fn max(&self) -> i32 {
        return *self.name.iter().max().unwrap();
    }
    
    pub fn min(&self) -> i32 {
        return *self.name.iter().min().unwrap();
    }
    
    pub fn first(&self) -> i32 {
        return *self.name.first().unwrap();
    }
    
    pub fn last(&self) -> i32 {
        return *self.name.last().unwrap();
    }
    ...
    pub fn avg(&self) -> f32 {
       return self.name.iter().sum::<i32>() as f32 / self.name.len() as f32
    }
    
    pub fn mode(&self) -> i32 {
        let mut occurrences = HashMap::new();
    
        for value in self.name.clone() {
